...
pub fn start_listener(tx: Sender<GlobalInputAction>, init_tx: SyncSender<Result<(), &'static str>>) {
    if ACTION_TX.set(tx).is_err() {
        let _ = init_tx.send(Err("A GlobalListener is already active for this process."));
        return;
    }

    let hook_handle = unsafe { SetWindowsHookExW(WH_KEYBOARD_LL, Some(low_level_keyboard_proc), None, 0) };
    ...
    if n_code >= 0 {
        ...
        if w_param_u == WM_KEYDOWN || w_param_u == WM_SYSKEYDOWN {
            if let Some(tx) = ACTION_TX.get() {
                if tx.send(GlobalInputAction::Keyboard).is_err() {
                    // The receiver has been dropped, so the message loop will terminate soon.
                }
            }
        }
    }
